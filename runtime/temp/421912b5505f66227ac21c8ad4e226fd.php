<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:44:"./application/admin/view/plugin\setting.html";i:1522317281;s:66:"I:\project\demo\oscshop2\application\admin\view\public\layout.html";i:1522317281;}*/ ?>
<!doctype html>
<html>
<head>
                ...
                <dd class="opt">
                    <?php if($config['type'] == 'select'): ?>
                        <select name="config[<?php echo $config['name']; ?>]">
                            <?php if(is_array($config['option']) || $config['option'] instanceof \think\Collection || $config['option'] instanceof \think\Paginator): if( count($config['option'])==0 ) : echo "" ;else: foreach($config['option'] as $o=>$option): ?>
                    ...
                    <?php endif; ?>
                    <p class="notic" <?php if($config['name'] == 'alipay_private_key'): ?>style="color:red;font-weight:bold"<?php endif; ?> ><?php echo (isset($config['description']) && ($config['description'] !== '')?$config['description']:$config['label']); ?></p>
                </dd>
            </dl>
            <?php endforeach; endif; else: echo "" ;endif; ?>
